...
/**
 * External function: probe a BBB recording's /capture/ playback page for a directly playable
 * media URL, so the grading client can render its own <video> with click-to-seek.
 *
...
/**
 * Cached server-side probe. The first call per recording fetches the BBB /capture/ HTML, regex-
 * extracts the <video src=…> URL, and persists the result in bbbext_advgrd_rec_probe. Later calls
 * read from the cache. Refresh is explicit (refresh=true).
 */
class probe_recording extends external_api {
    ...
    public static function execute(int $bbbid, string $recordingid, bool $refresh = false): array {
        ...
        // Extract <video src="…">. BBB's /capture/ player emits a plain HTML5 video element with
        // a direct media URL — usually .m4v on /presentation/<id>/video/webcams.m4v.
        ...
        if (parse_url($mediaurl, PHP_URL_SCHEME) === null) {
            // Resolve relative URL against the /capture/ page URL.
        ...
        return self::shape(self::persist($params['bbbid'], $params['recordingid'], $mediaurl, 'ok', $durationms));
    }
