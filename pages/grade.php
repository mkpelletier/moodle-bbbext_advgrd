...
use bbbext_advgrd\local\overlay;
...
// Annotation overlay: build it before $OUTPUT->header() so the editor's head_setup() lands
// in <head>. The renderer queues its own inline JS via $PAGE->requires; we just hold the
// returned HTML to echo after the rubric form.
$overlayhtml = overlay::render($bbbid, $userid, $recordingidparam !== '' ? $recordingidparam : null, $context, $user);
